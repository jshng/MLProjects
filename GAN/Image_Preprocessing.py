# -*- coding: utf-8 -*-
"""
Created on Mon Oct  7 09:28:09 2019
...
def random_affine_transform(x, theta_range = 0, tx_range = 0, ty_range = 0,
                            shear_range = 0, zx_range = 0, zy_range = 0, 
                            row_axis = 0, col_axis = 1, channel_axis = 2, 
                            fill_mode='nearest', cval=0., order=1):   
    if theta_range == 0:
        theta = 0
    ...
    if type(zx_range) == int:
        zx = 1
    else:
        zx    = np.random.uniform(zx_range[0],zx_range[1])
    if type(zy_range) == int:
        zy = 1
    else:
        zy    = np.random.uniform(zy_range[0],zy_range[1])
    
    x = apply_affine_transform(x, theta, tx, ty, shear, zx, zy, row_axis=0, 
